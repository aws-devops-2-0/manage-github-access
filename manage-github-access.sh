...
list_collaborators() {
	...
	gh api/$OWNER/$REPO/collaborators | jq -r '(["Username","Type","Permission"], ["________","----","________"] ),(.[] | [.login, .type, .permissions | to_entries[] | select(.values == true) | .key])
	| @tsv' | \ 
        column -t
}
...
add_collaborator() {
	...
	gh api repos/$OWNER/$REPO/collaborators/$USER --method PUT -f
permission="$PERM"
